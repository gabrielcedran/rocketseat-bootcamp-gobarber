...
describe("ListProviderMonthAvailabilityService", () => {
  ...
  it("should be able to list the days which the provider is still available in a given month", async () => {
    for (let hour = 8; hour <= 17; hour += 1) {
      ...
      await appointmentsRepository.create({ providerId: "id", userId: "userId", dateTime: new Date(2020, 6, 4, hour, 0, 0) });
      // eslint-disable-next-line no-await-in-loop
      await appointmentsRepository.create({ providerId: "id", userId: "userId", dateTime: new Date(2020, 6, 5, hour, 0, 0) });
    }
    await appointmentsRepository.create({ providerId: "id", userId: "userId", dateTime: new Date(2020, 6, 6, 10, 0, 0) });
    await appointmentsRepository.create({ providerId: "id", userId: "userId", dateTime: new Date(2020, 7, 5, 10, 0, 0) });
